...
RUN apt-get install git debhelper fakeroot devscripts \
    ...
    build-essential tzdata netcat libgnutlsxx28 uuid sudo fish -y && \
    apt-get clean && rm -Rf /var/cache/apt/ && rm -Rf /var/lib/apt/lists

RUN git clone --depth 1 --branch ${VERSION} https://github.com/GothenburgBitFactory/taskserver /tmp/taskserver
